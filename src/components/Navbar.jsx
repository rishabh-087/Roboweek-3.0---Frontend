...
const Navbar = () => {
  ...
  return (
    <>
      ...
      >
        <div className="backdrop-blur-lg bg-black/20 border border-cyan-500/30 rounded-2xl transition-all duration-500 ease-in-out hover:bg-black/30">
          <div className="flex flex-col items-center gap-3 py-3">
            <div
              className="w-12 h-12 flex items-center justify-center rounded-xl text-gray-300 transition-all duration-500 ease-in-out transform hover:text-cyan-400 hover:bg-cyan-500/10"
            >
              {isOpen ? (
            ...
            {navigation.map((item, index) => (
              <Link
                ...
                className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl text-gray-300 
                  ...
                  hover:text-cyan-400 hover:bg-cyan-500/10 hover:scale-105
              ...
              >
                ...
                {isOpen ? (
                  ...
                  <span className="absolute left-full ml-3 px-3 py-2 bg-black/80 text-cyan-400 text-base rounded-lg 
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-black/20 border-t border-cyan-500/30 p-4 flex justify-around z-50">
        {navigation.map((item) => (
          <Link key={item.name} to={item.href} className="flex flex-col items-center text-gray-300 hover:text-cyan-400 transition-colors duration-300">
